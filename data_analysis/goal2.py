...
actual_pos = [[]0 for x in range(6)]
goal_pos = [[]0 for x in range(6)]
goal_time = [[]0 for x in range(6)]
deviation = [[]0 for x in range(6)]
...
for line in test_file:
    if "GOAL" in line:
        for i in range(0,5):
            goal_pos[i].append(float(line.split(" ")[i+1]))
            goal_time[i].append(number_of_points)
    if "ACTUAL" in line:
        for i in range(0,5):
            actual_pos[i].append(float(line.split(" ")[i+1]))
...
for i in range(0,5):
    average_deviation[i] = avg_list(deviation[i])
