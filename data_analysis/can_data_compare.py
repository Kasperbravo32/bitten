...
for line in can_file:
    if can_id in line:
        ...
        if mux == "01":
            ...
            acc_x = float.fromhex(temp_str)
            acc_x = np.int16(acc_x) / float(10000)
            can_acc_x1.append(acc_x)

            temp_str = payload[8:10] + payload[6:8]
            acc_y = float.fromhex(temp_str)
            acc_y = np.int16(acc_y) / float(10000)
            can_acc_y1.append(acc_y)

            temp_str = payload[12:14] + payload[10:12]
            gyro = float.fromhex(temp_str)
            gyro = np.int16(gyro) / float(1000)
            gyro = math.degrees(gyro)
            can_gyro1.append(gyro)

            temp_str = payload[14:16]
            temp = float.fromhex(temp_str)
            temp = np.int8(temp)
            can_temp1.append(temp)
        ...
        if mux == "06":
            ...
            gyro_vert = np.int16(gyro_vert) / float(1000)
            gyro_vert = math.degrees(gyro_vert)
            can_gyro_vert1.append(gyro_vert)
            
can_file.close()

avg_acc_x1 = movingaverage(can_acc_x1, 1)
avg_acc_y1 = movingaverage(can_acc_y1, 1)

number_of_samples = len(avg_acc_x1)
for i in range(0, number_of_samples):
    angle = math.atan2(avg_acc_y1[i], avg_acc_x1[i])
    angle = math.degrees(angle) + 90
    if angle > 90:
        angle = angle - 180
    elif angle < -90:
        angle = angle + 180
    can_angle1.append(angle)
...
plt.subplot(212)
plt.grid(True)
plt.plot(can_angle1, color = "black")
# plt.title("Angle [degrees]")
plt.xlabel("Samples [n]")
plt.ylabel("Angle [degrees]")
# plt.ylim(-40, 5)
